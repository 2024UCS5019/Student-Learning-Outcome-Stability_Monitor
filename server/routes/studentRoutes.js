...
router.get("/:id/dashboard", auth, roles("Admin", "Faculty", "Student"), ctrl.getStudentDashboard);
router.put("/:id", auth, roles("Admin"), ctrl.updateStudent);
router.patch("/:id/block", auth, roles("Admin"), ctrl.blockStudent);
router.patch("/:id/unblock", auth, roles("Admin"), ctrl.unblockStudent);
